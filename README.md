...
the config file [config.toml](data_and_config/config.toml), so it points to a ollama server that you can access. Then, type `docker compose up -d`: this should fire up the demo and make it available at 
...
Images were acquired from a smartphone on a background with ArUCo Markers which facilitated segmenting and cropping of images using package [arucutter](https://github.com/joheli/arucutter.git). 
